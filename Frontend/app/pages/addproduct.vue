...
const value = ref('Choose a product category')
...
<template>
  <UForm
    ...
    @submit="onSubmit"
  >
   <div class=" rounded-lg p-6 shadow-sm border border-gray-400 w-full mx-auto h-133">
  <div class="mb-1 -mt-2">Add product</div><br>
  <div class=' gap-1'>
    <UFormField label="Title" name="customer" class='w-full mb-2'>
      <UInput v-model="state.customer" class="w-full"  />
    </UFormField>
    
   <span>Description</span>
   <br>
   <UTextarea color="neutral"  :rows="10" highlight class="mt-1 w-full" />
  </div><br>
  <span>Media</span><br>
   <UModal title="Modal with title" class="mt-1">
  ...
  <label class='font-medium text-gray-700 dark:text-gray-200' style="font-size: 14px;">Category</label><br>
   <UInputMenu v-model="value" arrow :items="items" class="mt-1 w-full mb-1"/>
